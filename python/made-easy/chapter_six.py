...
# ми, которые есть в Python изначально,
# либо мы создаем их сами с помощью классов Python.
#
# Для понимания того, что такое объект, нам пока будет достаточно знать,
# что это росто «фрагмент памяти», содержащий значение, а также некоторые
# связанныес объектом операции
...
# неизменяемый?
#
# Числа 1234, 3 . 1 4 1 5, 3+4j , OЫ l l , Decimal ( ) ,
# Fraction ( )- Неизменяемый
#
# Строки ' nilabh ' , "Bob ' s " Неизменяемый
#
# Списки [ 1 , [ 2 , ' three ' ] , 4 . 5 ] , list ( range ( lO) ) Изменяемый
#
# Словари { ' food ' : ' spam' , ' taste ' : ' yum' } , dict (hours:1 0 )
#  Изменяемый
#
...
# Объяснение:
#
# Когда вы присваиваете b = a, обе переменные начинают ссылаться на один
# и тот же объект (список [1, 2, 3]).
...
# Вложенные ссылки
#
# Объекты могут содержать ссылки на другие объекты. Например,
# списки могут хранить другие списки или словари, которые,
# в свою очередь, могут ссылаться на другие объекты
...
#
# Python использует механизм подсчёта ссылок для управления памятью.
# Каждый объект хранит количество ссылок на него.
# Когда счётчик ссылок объекта становится равным нулю,
...
# или встроенное имя Ellipsis. В логическом смысле эквивалентен тrue.
...
# массивами, например, в библиотеке NumPy.
# Он позволяет упростить код, заменяя несколько двоеточий (:):
# +
...
def print_args(x_y: int, y_x: int, z_x: int, *args: int) -> None:
    ...
    # логические значения ведут себя как значения О и 1 почти во всех случаях,
    # за исключением того, что при преобразовании в строку возвращаются строки
    # "True"
    ...
    # руктор bytes ( ) . Кроме того, байтовые строки можно декодировать в строки с
    #  помощью метода decode ( )
    ...
    # какие-то данные. Примеры встроенных функций: len ( ) и math. sin ( )
    # (math - это стандартный встроенный модуль).
...
#
# Методы в Python — это функции, которые принадлежат объектам (например,строки
# списки, словари). Они позволяют выполнять определённые операции с объектами.
...
# Вот несколько полезных методов для работы со списками:
