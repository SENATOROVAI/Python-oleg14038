...
# filter() — Возвращает итератор с отфильтрованными
# элементами итерируемого объекта.
#
...
#
# целое число, представляющее код Unicode этого символа. Например, ord
# ( ' а ' )
#
...
# Как и у всего остального, у создания (определения) функций есть
# собственный синаксис. Он весьма прост:
#
# def имя_функции ( ) :
#
#     тело_функции
...
#
# сле слова def следует имя функции, затем пара круглых скобок ( ) и
# двоеточие ( :).
#
...
# Это поможет другимпрограммистам, читающим ваш код, легко понять,
...
# В Python существует четыре основных способа передачи аргументов в функцию, и
# их можно комбинировать. Эти варианты позволяют передавать аргументы гибким
# образом и обеспечивают возможность работы с разным количеством и
# ипами аргументов.
...
# значение,
# функция-генератор возвращает последовательность значений ( объектитератор ).
# Слово return в таких функциях заменяется на yield. Рассмотрим это подробнее.
...
# представления элементов словаря. Он возвращает представление,
# состоящее из пар ключ-значение, которые могут быть перебраны в цикле
