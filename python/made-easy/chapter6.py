...
#
#  any() — Возвращает True,
# если хотя бы один элемент итерируемого объекта истинный.
#
# ascii() — Возвращает строковое представление объекта,
# содержащего только ASCII-символы.
#
...
#
# filter() — Возвращает итератор с
# отфильтрованными элементами итерируемого объекта.
#
...
#
# в таблице Unicode равна i. Например, chr (97) возвращает строку  'а', а chr
# (8 364)
#
...
#
# dir ([oЬjectJ]) при вызове без аргумента возвращает
# список имен всех локальных
#
...
#
# целое число, представляющее код Unicode этого символа.
# Например, ord ( ' а ' )
#
...
# Как и у всего остального, у создания (определения)
# функций есть собственный син
...
#
# сле слова def следует имя функции, затем пара круглых скобок
# ( ) и двоеточие ( :).
#
...
# образом и обеспечивают возможность работы с разным
# количеством и типами аргументов.

# 1. Позиционные аргументы (Positional arguments)
#
# Аргументы передаются в функцию в порядке их объявления.
# Эти аргументы обязательны, и порядок имеет значение
...
# Аргументы передаются по имени. В этом случае порядок не имеет значения,
...
#
# С помощью *args можно передавать функции переменное количество
# позиционных аргументов. Все переданные аргументы собираются в кортеж.
...
# Генератор - это особый класс функций. Вместо того,
# чтобы возвращать одно значение,
# функция-генератор возвращает последовательность значений
...
# Он возвращает представление, состоящее из пар ключ-значение,
#  которые могут быть перебраны в цикле
