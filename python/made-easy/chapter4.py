...
# своить». В Python «равно» записывается как ==. Мы увидим это позже, когда будем
...
#
# В программе на Python можно выделить такие части, как модули, выражения, операторы и объекты
...
# Каждый объект имеет уникальный идентификатор, который фактически является адресом объекта в памяти.
#
...
# Объяснение:
#
#     Когда вы присваиваете b = a, обе переменные начинают ссылаться на один и тот же объект (список [1, 2, 3]).
#     Изменение через переменную b приводит к изменению объекта, на который ссылается и a, так как это один и тот же объект в памяти.

# Ссылки в изменяемых и неизменяемых объектах
#
#     Изменяемые объекты (mutable objects): списки (list), словари (dict), множества (set) и объекты классов могут изменяться по ссылке.
...
#
# изменить, создаётся новый объект, а переменная просто перенаправляется на него.
...
# Вложенные ссылки
#
# Объекты могут содержать ссылки на другие объекты. Например, списки могут хранить другие списки или словари, которые, в свою очередь, могут ссылаться на другие объекты
...
#
# Python использует механизм подсчёта ссылок для управления памятью. Каждый объект хранит количество ссылок на него. Когда счётчик ссылок объекта становится равным нулю, объект может быть удалён сборщиком мусора.
...
# значением. Доступ к этому объекту осуществляется через литерал . . . или встро
# енное имя Ellipsis. В логическом смысле эквивалентен тrue.

# Заполнитель в функциях: Эллипсис может использоваться как заполнитель в функциях, когда вы планируете добавить код позже:

def some_function() -> None:
    """Выполняет действие без возврата значения."""


# Срезы в многомерных массивах: Эллипсис полезен при работе с многомерными массивами, например, в библиотеке NumPy. Он позволяет упростить код, заменяя несколько двоеточий (:):
...
# Аргументы функций: Эллипсис может использоваться для указания переменного количества аргументов в функции:
...
# Аннотации типов: В Python 3.10 и выше, эллипсис можно использовать в аннотациях типов для указания, что функция может принимать аргументы любого типа:
...
# Списковые выражения: Эллипсис может использоваться в списковых выражениях для создания последовательностей с пропущенными элементами:
...
# логические значения ведут себя как значения О и 1 почти во всех случаях, за ис
...
# пазон значений и обработка переполнений задаются на уровне базовой машиной архитектуры (и реализации С или Java). Python не поддерживает числа
...
# Комплексные числа в Python являются встроенным типом данных и могут быть представлены в виде a + bj, где a — действительная часть, а b — мнимая часть. 
#
# Здесь j обозначает мнимую единицу, аналогично математической записи, где обычно используется i.
#
...
# сать a [ i J .
#
# Последовательности также поддерживают операции срезов. Срез а [ i : j J выбираетвсе элементы с таким индексом k, что выполняется неравенство i <= k < j . 
#
# При использовании в выражении срез представляет собой последовательность того жетипа. 
#
...
# число в диапазоне от о до l0FFFF, а функция chr ( ) преобразует целое число в диапазоне от о до l 0 FFFF в соответствующий строковый объект длиной 1 .
...
#
# Байтовые массивы (bytearrays) в Python представляют собой изменяемые последовательности байтов
...
# какие-то данные. Примеры встроенных функций: len ( ) и math. sin ( ) (math - это стандартный встроенный модуль).
#
...
#
# Методы в Python — это функции, которые принадлежат объектам (например, строки, списки, словари). Они позволяют выполнять определённые операции с объектами.

# 1. Методы для строк (string)
# Строки — это неизменяемые последовательности символов. Вот несколько полезных методов для работы с ними:
...
#
# Списки — это изменяемые последовательности элементов. Вот несколько полезных методов для работы со списками:
...
#
# Словари — это коллекции пар "ключ-значение". Вот методы для работы со словарями:
...
#
# Множества — это коллекции уникальных элементов. Вот несколько методов для работы с множествами:
